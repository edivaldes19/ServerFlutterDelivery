...
VALUES
    (?, ?, ?, ?, ?, ?);
    `
    db.query(sql, [order.id_client, order.id_address, 'Pagado', Date.now(), new Date(), new Date()], (err, res) => {
        if (err) {
            console.log('ERROR', err)
            result(err, null)
        } else {
...
Order.updateToReady = (id_order, id_delivery, result) => {
    const sql = `
    UPDATE
    orders
SET
...
WHERE
    id = ?;
    `
    db.query(sql, [id_delivery, 'Preparado', new Date(), id_order], (err, res) => {
        if (err) {
            console.log('ERROR', err)
            result(err, null)
        } else {
...
WHERE
    id = ?;
    `
    db.query(sql, ['En camino', new Date(), id_order], (err, res) => {
        if (err) {
            console.log('ERROR', err)
            result(err, null)
        } else {
...
WHERE
    id = ?;
    `
    db.query(sql, ['Entregado', new Date(), id_order], (err, res) => {
        if (err) {
            console.log('ERROR', err)
            result(err, null)
        } else {
